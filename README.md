...
其中：
  公共电费分摊 = 公共电费 ÷ 房号内总人数（含尾差处理）
  空调电费分摊 = 套间空调电费 ÷ 套间内人数（含尾差处理）
```
...
→ 公共用电：550度，269.5元
→ 公共电费分摊：269.5÷6 = 44.92元/人
→ 每人总电费 = 44.92元 + 所在套间的空调费分摊
```

详细说明：[docs/V2_REFACTOR_GUIDE.md](docs/V2_REFACTOR_GUIDE.md)
...
6. **电表录入**：按房号折叠面板、总表+空调表批量录入、实时计算预览
7. **电表管理**：查询统计、公共用电展示、导出明细
8. **水费管理**：账单录入、分摊明细可编辑
9. **结算管理**：预检查、生成结算、锁定/解锁、汇总统计
